...
The advantage of using Terraform modules in your infrastructure as code (IaC) projects lies in improved organization, reusability, and maintainability. There is something called as `Terraform Registry` which has modules available publically. You can use them at your own risk.
- The terraform.tfvars file is usually not present in modules and the people using modules are supposed to provide it for their specific use case either by defining along with the module code or by having their own terraform.tfvars

Here are the key benefits of using Modules:
